		...
		r.prototype.init = function(e) {
			...
			try {
				var n = e.lang || "",
					...
					a = e.avatar || "mm",
					s = e.avatar_cdn || "";
				n && r && t.installLocale(n, r), t.locale = t.locale || p[e.lang || "zh-cn"], t.notify = e.notify || !1, t.verify = e.verify || !1, h.params = "?d=" + (i.indexOf(a) > -1 ? a : "mm"), h.hide = "hide" === a, h.cdn = !!s && s || h.cdn, g = e.path || g, e.guest_info = (e.guest_info || f).filter(function(e) {
				...
				AV.applicationId = null, AV.init({
					appId: c,
					appKey: u,
					region: d
				});
				for (var m = document.querySelectorAll(".valine-comment-count"), b = 0, x = m.length; b < x; b++) ! function(e, n) {
					var r = m[e];
					if (r) {
				...
				t.el = y, t.el.classList.add("v"), h.hide && t.el.classList.add("hide-avatar");
				...
				t.loading = {
					show: function(e) {
						...
						return e ? A.insertBefore(O, n[0]) : A.appendChild(O), t.nodata.hide(), t
					},
					hide: function() {
						var e = A.querySelector(".vloading");
						return e && A.removeChild(e), 0 === A.querySelectorAll(".vcard").length && t.nodata.show(), t
					}
				};
				...
				t.alert = {
					show: function(e) {
						...
						if (n.innerHTML = "" + r + (e && e.type && i), o.Event.on("click", $.querySelector(".vcancel"), function(e) {
								t.alert.hide()
							}), $.setAttribute("style", "display:block;"), e && e.type) {
							var a = $.querySelector(".vsure");
							o.Event.on("click", a, function(n) {
			...
			return t.equalTo("url", decodeURI(e)), 1 == arguments.length ? t.doesNotExist("rid") : t.equalTo("rid", arguments[1]), t.addDescending("createdAt"), t.addDescending("insertedAt"), t
			...
			var n = this;
			return t = t || {}, p[e] = JSON.stringify(Object.keys(p["zh-cn"])) == JSON.stringify(Object.keys(t)) ? t : void 0, n.locale = p[e] || p["zh-cn"], n
			...
			var t = this,
				...
				p = function(e) {
					...
					var n = s(t);
					d.comment = n, i.innerHTML = n;
				},
				f = u.data,
				mail_comment = "";
			for (var b in f) f.hasOwnProperty(b) && function(e, t) {
				var r = document.createElement("i");
				r.setAttribute("name", e), r.setAttribute("title", e), r.innerHTML = t, n.appendChild(r), o.Event.on("click", r, function(e) {
					var n = document.querySelector(".veditor");
					A(n, t), p(n)
				})
			}(b, f[b]);
			o.Event.on("click", r, function(e) {
			...
			var A = function(e, t) {
				...
				$ = function(e) {
					...
					return r || (r = document.createElement("div"), r.setAttribute("class", "vquote"), n.appendChild(r)), r
				},
				M = function n() {
					...
					s.limit(i), s.skip((r - 1) * i), s.find().then(function(e) {
						for (var l = e.length, c = 0; c < l; c++) ! function(n) {
							var r = e[n],
								i = r.id;
							C(r, t.el.querySelector(".vlist"), !0), s = t.Q(g, i), s.find().then(function(e) {
								var t = e.length;
								if (t)
									for (var n = 0; n < t; n++) C(e[n], $(i))
							})
						}(c);
						...
						d && o.Event.on("click", d, function(e) {
							u.innerHTML = "", n(++r)
						}), t.loading.hide()
					}).catch(function(e) {
			...
			var C = function(e, n, r) {
					...
					i.setAttribute("class", "vcard"), i.setAttribute("id", e.id);
				...
				L = function(e) {
					setTimeout(function() {
						e.offsetHeight > 180 && (e.classList.add("expand"), o.Event.on("click", e, function(t) {
				...
				T = function(e) {
					o.Event.on("click", e, function(t) {
						mail_comment = window.$(e).parent().next(".vcontent").html();
						var n = e.getAttribute("at"),
							r = e.getAttribute("rid"),
							i = e.getAttribute("mail");
						q.at = o.HtmlUtils.escape(n) + " ", q.rid = r, q.rmail = i, y.comment.setAttribute("placeholder", n), y.comment.focus()
					})
				},
				z = function() {
					...
					if (e) {
						...
						for (var r in n) {
							var i = n[r];
							t.el.querySelector(".v" + i).value = o.HtmlUtils.unescape(e[i]), d[i] = e[i]
						}
					}
				};
			z();
			var R = function() {
					...
					y.comment.setAttribute("placeholder", t.placeholder), q.at = "", q.rid = "", q.rmail = "", d.nick = "Anonymous", z(), i.innerHTML = "", i.setAttribute("style", "display:none;"), O.removeAttribute("v")
				},
				...
				U = function(e) {
					...
					var n = m.mail(d.mail),
						r = m.link(d.link);
					d.mail = n.k ? n.v : "", d.link = r.k ? r.v : "", t.notify || t.verify ? Q(j) : j()
				},
				P = function() {
					var e = new AV.ACL;
				...
				j = function() {
					B.setAttribute("disabled", !0), t.loading.show(!0);
					window.$.ajax({
						type: "post",
						url: "https://galaxymimi.com/api/phpmail/index.php",
						data: {
							address: q.rmail,
							nick: q.at.replace("@", ""),
							...
							reply_nick: d.nick,
							reply_comment: d.comment,
							url: location.href
						}
					});
					var e = AV.Object.extend("Comment"),
						n = new e;
					d.url = decodeURI(g), d.insertedAt = new Date, q.rid && (n.set("rid", q.rid), d.comment = q.at + d.comment);
					for (var r in d)
					...
					n.setACL(P()), n.save().then(function(e) {
						...
						try {
							if (q.rid) {
								...
								C(e, i, !0)
							} else n ? (r = Number(n.innerText) + 1, n.innerText = r) : t.el.querySelector(".vcount").innerHTML = '<span class="num">1</span> ' + t.locale.tips.comments, C(e, t.el.querySelector(".vlist"));
							d.mail && I({
								username: d.nick,
								mail: d.mail
								...
								mail: q.rmail
							}), B.removeAttribute("disabled"), t.loading.hide(), R()
						} catch (e) {
			...
			o.Event.off("click", B, U), o.Event.on("click", B, U), o.Event.on("keyup", document, function(e) {
		...
		(function(t) {
			! function(t) {
				...
				f._label = /(?:\\[\[\]]|[^\[\]])+/, f._title = /(?:"(?:\\"|[^"]|"[^"\n]*")*"|'\n?(?:[^'\n]+\n?)*'|\([^()]*\))/, f.def = c(f.def).replace("label", f._label).replace("title", f._title).getRegex(), f.bullet = /(?:[*+-]|\d+\.)/, f.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/, f.item = c(f.item, "gm").replace(/bull/g, f.bullet).getRegex(), f.list = c(f.list).replace(/bull/g, f.bullet).replace("hr", "\\n+(?=\\1?(?:(?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$))").replace("def", "\\n+(?=" + f.def.source + ")").getRegex(), f._tag = "(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b", f.html = c(f.html).replace("comment", /<!--[\s\S]*?-->/).replace("closed", /<(tag)[\s\S]+?<\/\1>/).replace("closing", /<tag(?:"[^"]*"|'[^']*'|\s[^'"\/>\s]*)*?\/?>/).replace(/tag/g, f._tag).getRegex(), f.paragraph = c(f.paragraph).replace("hr", f.hr).replace("heading", f.heading).replace("lheading", f.lheading).replace("tag", "<" + f._tag).getRegex(), f.blockquote = c(f.blockquote).replace("paragraph", f.paragraph).getRegex(), f.normal = p({}, f), f.gfm = p({}, f.normal, {
					...
					e = e.replace(/^ +$/gm, "");
					for (var n, r, i, o, a, s, l, c, u, d, p; e;)
				...
				v._scheme = /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/, v._email = /[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/, v.autolink = c(v.autolink).replace("scheme", v._scheme).replace("email", v._email).getRegex(), v._inside = /(?:\[[^\[\]]*\]|\\[\[\]]|[^\[\]]|\](?=[^\[]*\]))*/, v._href = /\s*<?([\s\S]*?)>?(?:\s+['"]([\s\S]*?)['"])?\s*/, v.link = c(v.link).replace("inside", v._inside).replace("href", v._href).getRegex(), v.reflink = c(v.reflink).replace("inside", v._inside).getRegex(), v.normal = p({}, v), v.pedantic = p({}, v.normal, {
					...
					for (var t, n, r, i, o = ""; e;)
						...
						else if (i = this.rules.autolink.exec(e)) e = e.substring(i[0].length), "@" === i[2] ? (n = s(this.mangle(i[1])), r = "mailto:" + n) : (n = s(i[1]), r = n), o += this.renderer.link(r, null, n);
						...
						else if (i = this.rules.code.exec(e)) e = e.substring(i[0].length), o += this.renderer.codespan(s(i[2].trim(), !0));
				...
				d.exec = d, h.options = h.setOptions = function(e) {
					...
					xhtml: !1,
					baseUrl: null
				}, h.Parser = a, h.parser = a.parse, h.Renderer = i, h.TextRenderer = o, h.Lexer = n, h.lexer = n.lex, h.InlineLexer = r, h.inlineLexer = r.output, h.parse = h, e.exports = h
