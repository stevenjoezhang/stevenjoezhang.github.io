	...
	EXIF.getData(target, function() {
		...
		if (FocalLength && FNumber && ExposureTime) {
			if (ExposureTime <= 0.25) {
				ExposureTime = '1/' + parseInt(1 / ExposureTime);
			}
			$(target).after(`<p class="image-caption">${FocalLength}mm f/${FNumber} ${ExposureTime}s</p>`);
		}
	});
}

$('.post-body img').each(function(i, o) {
	o.complete ? getExif(o) : $(o).on('load', function() {
		getExif(o);
	});
});
