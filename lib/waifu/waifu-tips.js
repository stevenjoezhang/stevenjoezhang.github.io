...
(function() {
	...
	function initWidget(waifuPath, apiPath) {
		...
		$(document).on("visibilitychange", function() {
			if (document.hidden) {
				...
				document.title = title[titleIndex].blur + " " + OriginTitile;
				if (titleTime) {
					clearTimeout(titleTime);
					titleTime = null;
				}
			} else {
				document.title = title[titleIndex].focus + " " + OriginTitile;
	...
	loadWidget(["/lib/waifu/waifu-tips.json", "https://galaxymimi.com/api/live2d"]);
